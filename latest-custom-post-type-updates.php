...
class TM_LCPTU {
	...
	public function _admin_enqueue_scripts( $hook ) {
		...
		wp_enqueue_script( 'tm-lcptu-admin', plugins_url( "js/tm-lcptu-admin-options{$min}.js", __FILE__ ), array( 'jquery' ) );
	}
...
if ( class_exists( 'tm_latest_cp_widget' ) && class_exists( 'WP_Widget' ) ):
	class tm_latest_cp_widget extends WP_Widget {
		...
		function update( $new_instance, $old_instance ) {
			...
			$instance['output_orderby'] = ( in_array( $new_instance['output_orderby'], array(
				'same',
				'ID',
				'author',
				'title',
				...
				'parent',
				'rand'
			) ) ) ? $new_instance['output_orderby'] : 'same';
			...
			$instance['date_format']        = ( in_array( trim( $new_instance['date_format'] ), array(
				'WP',
				'CUSTOM',
				'n/j/Y',
				'm/d/Y',
				'M. j, Y'
			) ) ) ? trim( $new_instance['date_format'] ) : 'WP';
							...
							if ( has_post_thumbnail( $id ) ) {
								...
								if ( $instance['thumbnail_format'] ) {
									if ( $instance['thumbnail_format'] != '0' ) {
										...
										$image = wp_get_attachment_image_src( get_post_thumbnail_id( $id ), array(
...
if ( ! function_exists( 'tm_lcptu_get_terms' ) ) {
	function tm_lcptu_get_terms(
		$taxonomy, $field_name = '', $checked = array(
		'post_in'  => 'IN',
		'term_ids' => array()
	)
	) {
